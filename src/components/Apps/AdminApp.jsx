        ...
        <div className="space-y-4 animate-in fade-in duration-300">
            ...
            <div className="overflow-hidden border border-white/10 rounded-xl">
                <table className="w-full text-left">
                    <thead className="bg-white/5 text-indigo-300 text-sm">
                        <tr>
                    ...
                    <tbody className="divide-y divide-white/5 text-gray-300">
                        {users.map(u => (
                            <tr key={u.username} className="hover:bg-white/5 transition-colors">
                                ...
                                <td className="p-4">
                                    <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${u.is_admin ? 'bg-indigo-600 text-white' : 'bg-gray-600/50 text-gray-400'}`}>
                    ...
                    <div key={f.id} className="bg-indigo-500/5 border border-indigo-500/20 p-4 rounded-xl flex justify-between items-start gap-4">
                        <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                                <span className="text-indigo-300 font-bold font-mono">@{f.user}</span>
                                <span className="text-[10px] text-gray-500 italic">{f.timestamp}</span>
                            </div>
                            <p className="text-gray-200 text-sm leading-relaxed">{f.content}</p>
                        </div>
                        <button onClick={() => deleteFb(f.id)} className="text-gray-600 hover:text-red-400 p-2">
                            <Trash2 size={18} />
                        </button>
                    </div>
                ))}
            ...
            <div className="grid grid-cols-2 gap-4">
                {apps.map((app, idx) => (
                    <div key={idx} className="bg-white/5 border border-white/10 p-4 rounded-xl flex justify-between items-center">
                        <div>
                            <h4 className="font-bold text-indigo-300">{app.name}</h4>
                            <p className="text-xs text-gray-500">by @{app.owner}</p>
                        </div>
                        <button
                            onClick={() => togglePublic(app.owner, app.name, app.is_public)}
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${app.is_public ? 'bg-green-600 text-white' : 'bg-white/10 text-gray-400'}`}
                        >
                ...
                {apps.length === 0 && <div className="col-span-2 text-center py-12 text-gray-600 italic">No custom apps found.</div>}
            </div>
        </div>
    );
};
